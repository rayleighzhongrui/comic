...
var Modality;
(function(Modality3) {
  Modality3["MODALITY_UNSPECIFIED"] = "MODALITY_UNSPECIFIED";
  Modality3["TEXT"] = "TEXT";
  Modality3["IMAGE"] = "IMAGE";
  Modality3["AUDIO"] = "AUDIO";
  Modality3["VIDEO"] = "VIDEO";
})(Modality || (Modality = {}));
var ModelStage;
...
var getPlaceholderImage = (width = 1024, height = 1024) => {
...
var geminiService = {
   ...
   */
  checkAndOpenApiKeyDialog: async (model) => {
    const paidModels = ["gemini-3.1-flash-image-preview", "gemini-3-pro-image-preview", "imagen-4.0-generate-001", "veo-3.1-fast-generate-preview", "veo-3.1-generate-preview", "lyria-3-clip-preview", "lyria-3-pro-preview"];
   ...
   */
  generateReferenceImage: async (prompt, model = "imagen-4.0-generate-001" /* IMAGEN_4 */) => {
   ...
   */
  generateComicPanels: async (prompt, referenceImages, aspectRatio, model = "gemini-2.5-flash-image" /* GEMINI_2_5_FLASH */) => {
    ...
    if (model === "gemini-3.1-flash-image-preview" /* GEMINI_3_1_FLASH */) {
      config.imageConfig = {
        aspectRatio,
        imageSize: "1K"
      };
   ...
   */
  editComicPanel: async (prompt, originalImage, maskImage, model = "gemini-2.5-flash-image" /* GEMINI_2_5_FLASH */, referenceImage) => {
  ...
  extendComicPanel: async (storyText, imageOnCanvas, maskImage, model = "gemini-2.5-flash-image" /* GEMINI_2_5_FLASH */) => {
   ...
   */
  extractStyleFromImage: async (imageBase64) => {
    const prompt = `
    ...
    const contentRequest = {
      model,
      // Vision capabilities available in standard model
      contents: {
        parts: [
          { text: prompt },
   ...
   */
  generateStoryContinuation: async (previousStory, panelCount, layoutDescription, availableCameraShots, allCharacters, initialScenes, mainCharacters, relationshipsPrompt, pageOutline, contextImage) => {
    ...
    try {
      ...
      if (parsedResult.panel_details && Array.isArray(parsedResult.panel_details)) {
        ...
        const finalDetails = Array.from({ length: panelCount }, (_, i) => {
          ...
          const characterIds = (detail.characters || []).map((name) => charNameToId.get(name)).filter(Boolean);
          return {
